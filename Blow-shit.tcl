...
namespace eval ::ngBot::plugin::Blow {
	...
	proc GetTopic {channel} {
		...
		if {[IsTrue [${ns}::matchChan $channel]] && [string equal [lindex $topic 0] "+OK"]} {
			set topic [fishdance::decrypt $key [lindex $topic 1]]
		}
		return $topic
	}
	...
	proc put_encrypted {target type command text key {option ""}} {
		...
		if {![string equal $key ""]} {
			if {$option == ""} {
				put${type}2 "$command $target :+OK [fishdance::encrypt $key $text]"
			} else {
				put${type}2 "$command $target :+OK [fishdance::encrypt $key $text]" $option
			}
		} else {
